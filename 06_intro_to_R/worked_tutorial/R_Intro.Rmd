...
```{r}
# install.packages(pkgs = "~/Downloads/MoTrPAC-MotrpacRatTraining6moData-v2.1.0-0-gf831a4f.tar.gz", 
#   repos = NULL, 
#   type = "source")
```
...
```{r, solution=TRUE}
object_sum <- object_five + object_six
object_sum
```
...
```{r, solution=TRUE}
obj_comb <- c(object_five, object_six)
obj_comb
```
...
```{r, solution=TRUE}
pheno_deselect <- rat_pheno %>% select(!c("bid", "viallabel"))
dim(pheno_deselect)
```
...
```{r, solution=TRUE}
arrange(pheno_select, desc(weight))
```
...
```{r, solution=TRUE}
pheno_filter <- pheno_select %>%
  filter(sex=="male") %>%
  filter(weight > 300)

dim(pheno_filter)
summary(pheno_filter$weight)
```
...
pheno_bonus <- pheno_mutate %>%
  group_by(sex) %>%
  summarize(avg_vo_max1 = mean(vo_max1))
...
thresh <- mean(pheno_mutate$vo_max2, na.rm = TRUE)

pheno_bonus2 <- pheno_mutate %>%
  filter(sex == "female") %>%
  filter(vo_max2 > thresh)

dim(pheno_bonus2)

cat("VO_Max2 Threshold is:", thresh, "\n")
summary(pheno_bonus2$vo_max2)
```
...
rat_pheno_plasma <- rat_pheno %>%
  filter(tissue == "PLASMA")

dim(rat_pheno_plasma)
```
...
pheno_bonus3 <- pheno_select %>%
  mutate(vo2_improve = round((vo_max2 - vo_max1)/vo_max1 * 100, 2))

pheno_bonus3 %>%
  filter(vo2_improve == max(vo2_improve, na.rm = TRUE)) %>%
  select(pid, vo2_improve) %>%
  unique()
```
...
```{r, solution=TRUE}
ggplot(pheno_dist, aes(x=sex)) + geom_bar()
```
...
```{r, solution=TRUE}
ggplot(pheno_dist, aes(x = vo_max1)) + geom_histogram()
```
...
```{r, solution=TRUE}
ggplot(pheno_dist, aes(x = vo_max1)) + geom_histogram(binwidth = 5)
```
...
```{r, solution=TRUE}
ggplot(pheno_dist, aes(x = vo_max1)) + geom_density()
```
...
```{r, solution=TRUE}
ggplot(pheno_dist, aes(x = sex, y = vo_max1)) + geom_boxplot()
```
...
```{r}
ggplot(pheno_dist, aes(x=vo_max1, y=vo_max2, color = sex)) + geom_point() 
```

Where would we add `shape=group` in the code block below:

```{r}
ggplot(pheno_dist, aes(x=vo_max1, y=vo_max2, shape = group)) + geom_point() 
```

Where would we add `size=weight` in the code block below:

```{r}
ggplot(pheno_dist, aes(x=vo_max1, y=vo_max2, size = weight)) + geom_point() 
```
...
```{r}
ggplot(pheno_dist, aes(x=vo_max1, y=vo_max2)) + 
  geom_point(size = 12, color = "red", fill = "pink", shape = 22, alpha = 0.7) 
```
...
```{r}
ggplot(pheno_dist, aes(x=vo_max1, y=vo_max2, color = sex)) + 
  geom_point() +
  geom_smooth(method = "lm")
```
...
ggplot(pheno_long, aes(x = training_session, y = vo_mean, color = group)) +
  geom_point() +
  facet_wrap(~sex)
...
pheno_exp <- inner_join(pheno_mutate, watsc_rna_wide, by = "viallabel")
pheno_exp
...
```{r, solution=TRUE}
TRNSCRPT_WATSC_DA %>%
  mutate(significant = ifelse(adj_p_value < 0.5,
                            "TRUE", 
                            "FALSE")) %>%
  ggplot(., aes(x = logFC, y = -log10(adj_p_value), color = significant)) +
  geom_point(shape = 1) +
...
```{r, solution=TRUE}
ggplot(pheno_exp, aes(x = group, y = Gak)) +
  geom_boxplot() +
  facet_wrap(~sex)
```
